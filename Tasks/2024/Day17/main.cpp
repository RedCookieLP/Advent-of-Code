...
static std::string runProgram(const std::vector<Instruction>& program, uint64_t initialA, uint64_t initialB, uint64_t initialC);
...
void finalize()
{
	std::string outputStr = runProgram(s_program, s_registerA, s_registerB, s_registerC);
...
void finalize()
{
	...
	uint64_t originalA = s_registerA;
	...
	std::cout << "Let's try running the program with it!" << std::endl;
	std::string outputStr = runProgram(s_program, finalA, s_registerB, s_registerC);
	std::cout << "The string generated by the program reads \"" << outputStr << '"' << std::endl;
	std::cout << "Does this match the program's code? " << (outputStr == s_programStr ? "yes" : "no") << std::endl;
}
...
static uint64_t findA(uint64_t index, const std::vector<uint8_t>& targets, uint64_t currentA) noexcept
{
	...
	for (uint8_t possibleA = 0u ; possibleA < 8u ; possibleA++)
	{
...
static std::string runProgram(const std::vector<Instruction>& program, uint64_t initialA, uint64_t initialB, uint64_t initialC)
{
	s_registerA = initialA;
	s_registerB = initialB;
	s_registerC = initialC;

	// Just run the program and wait for it to come to a stop (aka. for the IP to go out of scope)
	std::string outputStr{};
	for (uint64_t regIP = 0u ; regIP < program.size() ; regIP++)
	{
		const Instruction& instr = program[regIP];
		...
		switch (instr.opCode)
		{
			...
			case OpCode::BST:
			{
				s_registerB = (operand.asCombo() % 8u);
			}
			break;
			case OpCode::JNZ:
			{
			...
			case OpCode::OUT:
			{
				if (!outputStr.empty())
					outputStr += ',';
				outputStr += std::to_string(operand.asCombo() % 8u);
			}
			break;
			case OpCode::BDV:
			{
				s_registerB = s_registerA >> operand.asCombo();
			}
			break;
			case OpCode::CDV:
			{
				s_registerC = s_registerA >> operand.asCombo();
			}
			break;
		default:
			break;
